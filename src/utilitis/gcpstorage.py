...
def copy_blob(
    ...
    if copy_if_exists:
        ...
        try:
            ...
            print(
                f"""Blob {source_blob.name} in bucket {source_bucket.name} \
                  copied to blob {blob_copy.name} in bucket \
                    {destination_bucket.name}"""
            )
        except TypeError:
            print(
                f"""Blob {source_blob.name} doesn't exist in destination.\
                   Creating new object"""
            )
            blob_copy = source_bucket.copy_blob(
                source_blob,
        ...
        try:
            ...
            print(
                f"""Blob {source_blob.name} already exists in destination.\
                   Skipping copy."""
            )
